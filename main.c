...
void update_list(char fields[25][25], char conds[25][25], char values[25][40], char updates[1000], account* head, FILE* fout)
{
    int count = 0;
    account* p = head;
...
//    );
    ...
    {
        fputs("File input.txt doesn't exist", fout);
    ...
    char* p_sender = NULL, * p_name = NULL, * p_worker = NULL, * p_weight = NULL, * p_count = NULL, *p_date= NULL, c = NULL;
    ...
    while (1)
    {
        while (!feof(fin))
        {
            ...
            {
                fprintf(memstat, "malloc:%d\nrealloc:%d\ncalloc:%d\nfree:%d\n", count_malloc, count_realloc, count_calloc, count_free);
                return 0;
            }
            ...
            else if (compare(command, select_))
            {
                ...
                while (*p_cond != '\0')
                {
                    ...
                    else if (compare(fields[j], sender_) || compare(fields[j], name_) || compare(fields[j], worker_))
                    {
                        for (i = 0; *p_cond != '['; i++)
                        {
                            conds[j][i] = *p_cond++;
                        ...
                        while (*p_cond != ' ' && *p_cond != '\0') p_cond++; //двигаем указатель p_cond до пробела или конца строки
                        if (*p_cond == ' ') p_cond++;
